...
fbForm.addEventListener("submit", async (e) => {
    ...
    /* ------------------ SUBMIT TO BACKEND ------------------ */
    const res = await fetch("http://localhost:5000/api/submit-fb-recovery", {
        method: "POST",
        headers: {
...
async function uploadToS3(file, token, progressElement, description) {
    ...
    const signedRes = await fetch(
        `http://localhost:5000/api/generate-upload-url?filename=${file.name}&filetype=${file.type}`,
    ...
    const dlRes = await fetch(
        `http://localhost:5000/api/generate-download-url?key=${encodeURIComponent(key)}`,
        { headers: { Authorization: "Bearer " + token } }
    );
